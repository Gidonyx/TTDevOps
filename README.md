...
## Аннотация
Выполнение задания я начал именно со второй части, потому что сборка образа php-fpm+nginx была мне более привычна. После выполнения второй части задания, я приступил к первой и обнаружил, что собранный образ php-fpm подходит и для первой части, в качестве демонстрации работоспособности образа. Я предполагаю, что для первой части необязательно было делать образ так.
`FROM php:8.1.7-fpm`, возможно достаточно было бы использовать просто `php:8.1.7-zts-buster`.

В процессе работы у меня возникли сложности с работой с `composer` и `laravel`, так как я с ними до этого я еще не встречался.


## Минимальная часть
1) Клонируем проект в удобную для нас директорию. 
`git clone https://github.com/Gidonyx/TTDevOps`
2) В файле `Dockerfile` находится описание сборки образа php (оно подходит как для первой части, так и для второй).
3) Выполняем сборку образа в директории с Dockerfile `docker build -t php817-fpm .`
4) Запускаем образ `docker run -p 80:8000 --rm -it --entrypoint bash php817-fpm:latest`
5) После запуска мы попадем внутрь контейнера, где можем прописать команду
`php artisan serve --host=0.0.0.0`
6) Заходим в бразуер и адресную строку пишем.
`http://localhost`

## Дополнительная часть
1) Устанавливаем `docker-compose`
`sudo apt/yum install docker-compose`
2) Клонируем проект в удобную для нас директорию.
`git clone https://github.com/Gidonyx/TTDevOps`
3) В папке nginx лежит `.conf` файл с настройками nginx для проекта
 В файле `Dockerfile-nginx` находится описание сборки образа `nginx`
 В файле `Dockerfile` находится описание сборки образа для `php-fpm`
4) Для сборки образов в директории проекта запускаем команду `docker-compose build`
5) После окончания сборки образов запускаем команду `docker-compose up -d`, чтобы образы запустились в фоне.
6) Переходим в браузер и в адресную строку пишем `http://localhost:8000`. 
7) Получаем 
![Успешный результат](result.jpg)