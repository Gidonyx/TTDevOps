...
## Аннотация
Выполнение задания я начал именно со второй части, потому что сборка образа php-fpm+nginx была мне более привычна. После выполнения второй части задания, я приступил к первой и обнаружил, что собранный образ php-fpm подходит и для первой части. Я предполагаю, что для первой части необязательно было делать образ так.
`FROM php:8.1.7-fpm`, возможно достаточно было бы использовать просто `8.1.7-zts-buster`.
